...
namespace CompanyAPI.DAL.Reprostaories
{
    public  class EmployeeRepo : IContextRepo<Employee>
    {
        ...
        {
            IEnumerable<Employee> employees =await dbContext.Employees.Include("departement").ToListAsync();
            return employees;
        }

        public async Task<Employee> getItemById(int id)
        {
            Employee? employee = await dbContext.Employees.Include("departement").
